...
class BQSP(keras.layers.Layer):
    ...
    def call(self, th):
        ...
        # tiled up X rotations (input W(x)) th1
        px = tf.constant([[0.0, 1], [1, 0]], dtype=tf.complex64)
        px = tf.expand_dims(px, axis=0)
        px = tf.repeat(px, [batch_dim], axis=0)

        rot_x_arg = tf.complex(real=0.0, imag=th1)
        rot_x_arg = tf.expand_dims(rot_x_arg, axis=1)
        rot_x_arg = tf.tile(rot_x_arg, [1, 2, 2])

        wx1 = tf.linalg.expm(tf.multiply(px, rot_x_arg))

        # tiled up X rotations (input W(x)) th2
        px = tf.constant([[0.0, 1], [1, 0]], dtype=tf.complex64)
        px = tf.expand_dims(px, axis=0)
        px = tf.repeat(px, [batch_dim], axis=0)

        rot_x_arg = tf.complex(real=0.0, imag=th2)
        rot_x_arg = tf.expand_dims(rot_x_arg, axis=1)
        rot_x_arg = tf.tile(rot_x_arg, [1, 2, 2])

        wx2 = tf.linalg.expm(tf.multiply(px, rot_x_arg))
        ...
        z_rotations = []
        for k in range(self.poly_deg * 2 + 1):
            phi = self.phis[k]
...
def construct_bqsp_model(poly_deg):
